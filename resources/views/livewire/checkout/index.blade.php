        ...
        <div class="lg:col-span-2">
            <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-sm overflow-hidden mb-6">
                <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800">
                    <h2 class="text-xl font-semibold text-zinc-900 dark:text-zinc-100">{{ __('Order Items') }} ({{ count($cartItems) }})</h2>
                </div>
                
                <!-- Cart Items Section -->
                <div class="divide-y divide-zinc-200 dark:divide-zinc-800">
                    @foreach ($cartItems as $item)
                        <div class="p-5 flex flex-col sm:flex-row items-start gap-5 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                            <!-- Product Image and Info -->
                            <div class="flex items-start gap-4 w-full sm:w-auto">
                                <div class="flex-shrink-0 bg-zinc-100 dark:bg-zinc-800 rounded-lg overflow-hidden w-20 h-20 p-0 shadow-sm">
                                    ...
                                    <div class="min-h-[24px] mt-2">
                                        @if($item['product']->stock < $item['quantity'])
                                            @if ($item['product']->stock > 0)
                                                <div class="text-sm text-amber-600 dark:text-amber-500 flex items-center">
                                                    <flux:icon name="alert-triangle" class="w-4 h-4 mr-1.5"/>
                                                    <span>The stock is only {{ $item['product']->stock }}. More than {{ $item['product']->stock }} will result in a slight delay.</span>
                                                </div>
                                            @else
                                                <div class="text-sm text-red-600 dark:text-red-400 flex items-center">
                                                    <flux:icon name="x-circle" class="w-4 h-4 mr-1.5"/>
                                                    <span>The product is out of stock, so it will result in a slight delay.</span>
                                                </div>
                                            @endif
                                        @endif
                                    </div>
                                    ...
                                    @if($item['showDiscount'])
                                        <div class="flex flex-col">
                                            ...
                                            <div class="inline-flex items-center justify-end mt-1">
                                                <span class="text-xs px-2 py-0.5 bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400 rounded-full flex items-center">
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-sm p-6 sticky top-6">
                ...
                <div class="space-y-5">
                    ...
                    <button wire:click="processPayment" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-4 px-6 rounded-lg transition-colors flex items-center justify-center space-x-2 shadow-sm">
