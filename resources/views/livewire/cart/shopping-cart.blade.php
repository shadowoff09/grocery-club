    ...
    @else
        <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-sm overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800">
                <h2 class="text-xl font-semibold text-zinc-900 dark:text-zinc-100">Shopping Cart ({{ $cartItems->count() }} items)</h2>
            </div>
                ...
                @foreach($cartItems as $item)
                    <div class="p-4 flex items-center gap-4">
                        ...
                        <div class="flex items-center gap-4">
                            <div class="flex items-center border border-zinc-200 dark:border-zinc-700 rounded-lg">
                                <button wire:click="updateQuantity({{ $item['product']->id }}, 'decrease')"
                                        class="p-1.5 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
                                    <flux:icon name="minus" class="w-3.5 h-3.5" />
                                </button>
                                <span class="w-10 text-center font-medium text-zinc-900 dark:text-zinc-100">
                                    {{ $item['quantity'] }}
                                </span>
                                <button wire:click="updateQuantity({{ $item['product']->id }}, 'increase')"
                                        class="p-1.5 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
